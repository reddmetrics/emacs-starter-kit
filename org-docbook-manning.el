...
(defcustom org-export-docbook-article-header
  "<chapter xmlns=\"http://www.manning.com/schemas/book\"
          xmlns:ns=\"http://www.manning.com/schemas/book\"
          xmlns:xs=\"http://www.w3.org/2001/XMLSchema\"
          xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">"
  "Chapter header of DocBook XML files."
  :group 'org-export-docbook
  :type 'string)
              ...
              (if (or (get-text-property (match-beginning 2) 'org-protected line)
                ...
                (let* ((num (match-string 2 line))
                  ...
                  (if (assoc num footref-seen)
                    ...
                    (setq line (replace-match
                                (concat
                                 (format "%s<footnote id=\"%s%s\"><para>%s</para></footnote>"
                                         (match-string 1 line)
  ...
  (let (table-with-label)
    ...
    (setq table-with-label
          (if (string-match
               "^<table \\(\\(.\\|\n\\)+\\)<a name=\"\\(.+\\)\" id=\".+\"></a>\n\\(\\(.\\|\n\\)+\\)</table>"
               table)
              (replace-match (concat "<table id=\"" (match-string 3 table) "\" "
...
(org-add-link-type
 ...
 (lambda (path desc format)
   (cond
    ((eq format 'html)
     (format "<xref linkend=\"%s\"/>" path))
    ((eq format 'latex)
     (format "\\ref{%s}" path)))))

(provide 'org-docbook-manning)
